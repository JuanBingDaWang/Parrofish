...
2. 阅读用户给出的写作任务描述
3. 如果任务提供了 KB 检索结果，用它验证角度可行性；无事实构思模式则跳过此步
4. 按作者的思考方式明确目的、受众、预期效果和切入角度，并如实说明事实依据边界

选题原则：
- 角度必须体现作者的独特思维方式，而不是套用与文体无关的通用模板
- 知识库事实写作的角度必须能被 KB 证据支撑；无事实构思只能形成观点、框架和条件性假设
...
2. 知识库事实写作阅读候选检索结果；无事实构思模式不使用候选事实证据
...
- 有证据的任务应映射候选证据；无事实构思的所有节点都只能是诠释、论证、问题或建议
...
- 每条论断必须标注类型：fact（事实，需 source_key）、
  interpretation（分析推理，无需 source_key）。禁止新生成 common 类型
...
def topic_selection_messages(
    *,
    ...
    conceptual = context.generation_options.evidence_mode == "conceptual_only"
    request = {
        "task": "明确非虚构创作意图",
        "rules": [
            ...
            (
                "当前是无事实构思模式：不检索知识库，只形成观点、问题、框架、建议和条件性假设。"
                if conceptual
                else "角度必须能被 KB 现有证据支撑；如果证据不足，诚实标注薄弱方面。"
            ),
...
def framework_messages(
    *,
    ...
    evidence_rules = (
        [
            "当前是无事实构思模式：candidate_source_keys 必须为空。",
            "所有节点只能安排观点、问题、框架、建议、方法或条件性假设，不得安排具体外部事实。",
        ]
        if conceptual
        else [
...
def drafting_messages(
    *,
    ...
    evidence_rules = (
        [
            "当前为无事实构思模式，证据包必须为空。",
            "所有 claims 的 type 只能是 interpretation；禁止 fact 和 common。",
            "不得使用任何 source_key 或 [S数字] 引用标记。",
            "只能写观点、问题、框架、建议、方法和明确标注的条件性假设。",
            "不得写数据、年份、比例、法规、人物事件、引文等需要外部来源核验的具体事实。",
        ]
        if conceptual_only
        else [
            "事实性陈述只能来自证据包中的逐字摘录，且必须绑定对应的 source_key。",
            "每条论断必须标注 type：fact / interpretation；禁止新生成 common。",
