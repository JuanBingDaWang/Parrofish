...
def _document_form_rule(document_form: DocumentForm) -> str:
    ...
    if document_form == "short_text":
        return "目标产物是紧凑的短文或摘要，不得套用完整论文的多章结构。"
    return "目标产物是完整论文，应形成层次清楚、可持续推进的论证结构。"

_PERSONA_HAT_RULES = """你是被蒸馏出的作者本人，正在用他的思维方式、论证风格和表达习惯进行学术写作。
你必须以第一人称视角思考和组织论证，但输出时使用规范的学术表达。
...
TOPIC_SELECTION_SYSTEM = f"""你是被蒸馏出的学术作者，正在为一个新的写作任务确定中心论旨。
...
FRAMEWORK_SYSTEM = f"""你是被蒸馏出的学术作者，正在为已确定的中心论旨构建与文稿形态相称的论证骨架。
...
DRAFTING_SYSTEM = f"""你是被蒸馏出的学术作者，正在按论证单元撰写学术文本草稿。
...
POLISHING_SYSTEM = f"""你是被蒸馏出的学术作者，正在对已通过事实核对的学术文本草稿进行文风打磨。
    ...
    *,
    ...
    request = {
        ...
        "rules": [
            ...
            "不得引入证据包之外的任何事实、数据、引文、书名、人名、年份或事件。",
            "按 persona 的论证风格和表达习惯组织内容，但保持规范的学术表达。",
...
TERM_CONSISTENCY_SYSTEM = f"""你是中立的学术文本审查者，负责检查整篇文稿的术语一致性。
...
STRUCTURE_REVIEW_SYSTEM = f"""你是中立的学术编辑，负责对已完成初稿的文稿进行结构审查。
...
GLOBAL_POLISH_SYSTEM = f"""你是一位资深学术编辑，正在对一篇完整学术文稿做最后一次全局通读和润色。
