...
class StepSettingsDialog(QDialog):
    ...
    def __init__(
        ...
        *,
        ...
        self.timeout_input.setToolTip("每次流式或非流式尝试都独立获得这个时限")
        form.addRow("单次尝试上限:", self.timeout_input)
        self.total_timeout_input = QSpinBox()
        self.total_timeout_input.setRange(0, 21600)
        self.total_timeout_input.setSingleStep(60)
        self.total_timeout_input.setSpecialValueText("继承全局")
        self.total_timeout_input.setSuffix(" 秒")
        self.total_timeout_input.setToolTip(
            "只决定能否启动下一次尝试；已经启动且仍在输出的请求不会因此被截断"
        )
        form.addRow("整项调用总预算:", self.total_timeout_input)
        layout.addLayout(form)
    ...
    def _set_values(self, config: ChatStepConfig) -> None:
        ...
        self.total_timeout_input.setValue(config.total_timeout_seconds or 0)
    ...
    def _config(self) -> ChatStepConfig:
        return ChatStepConfig(
            ...
            total_timeout_seconds=self.total_timeout_input.value() or None,
        )
