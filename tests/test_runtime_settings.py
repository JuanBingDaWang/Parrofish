...
def test_runtime_settings_persist_and_apply_to_client(settings) -> None:
    ...
    try:
        ...
        service.save_provider(
            "bocha",
            secret="bocha-secret",
            base_url="https://search.example.invalid/v1",
        )
        context.set_web_search_result_count(7)
        context.set_siliconflow_request_timeout(1200)
        context.set_siliconflow_total_timeout(3000)
        context.set_siliconflow_stream_idle_timeout(240)
        ...
        assert secrets.values["bocha-api-key"] == "bocha-secret"
        assert context.bocha.transport.base_url == "https://search.example.invalid/v1"
        assert context.get_web_search_result_count() == 7
        assert context.get_siliconflow_request_timeout() == 1200
        assert context.get_siliconflow_total_timeout() == 3000
        assert context.get_siliconflow_stream_idle_timeout() == 240
        assert context.siliconflow.transport.default_request_timeout_seconds == 1200
        assert context.siliconflow.default_total_timeout_seconds == 3000
        assert context.siliconflow.transport.stream_idle_timeout_seconds == 240
    ...
    try:
        ...
        assert reopened.bocha.transport.base_url == "https://search.example.invalid/v1"
        assert reopened.get_web_search_result_count() == 7
        assert reopened.get_siliconflow_request_timeout() == 1200
        assert reopened.get_siliconflow_total_timeout() == 3000
        assert reopened.get_siliconflow_stream_idle_timeout() == 240
    finally:
        reopened.close()
